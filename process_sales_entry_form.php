...
if (isset($_POST) && !empty($_POST)) {
  ...
  if (isset($_POST["event-id"])) {
    ...
    if (!isset($_POST['datepicker']) || empty($_POST['datepicker']) || !isset($_POST['event_type']) || empty($_POST['event_type'])) {
      ...
      header('Location: ' . $domain . '/view_events.php?eventID=' . $event_id);
    }
  }
}
...
function populate_form_with_saved_data($event_id) {
  ...
  while($row = $result->fetch_assoc()){
    ...
    $data['event_name'] = htmlentities($row['name']);
  ...
  while($row = $result->fetch_assoc()){
    ...
    $data['retail_price'][$row['stock_id']] = $row['price'];
  }
  return $data;
}
...
function save_event_sales_data($_POST) {
  ...
  //Check to see if this a new event or of we are updating an event
  ...
  //We build a mapping of stock ids to their retail price
  //If we are updating a record we need to get the retail price at the 
  //time, not now

  //Fetch stock id's from the database
  $sql = "SELECT id, retail_price, cost_price FROM `stock`"; //new record
  if (isset($update_event)) {
    $sql = "SELECT stock_id AS id, price AS retail_price, cost as cost_price FROM sales_record WHERE event_record_id=$update_event"; //existing record
  }
  if(!$result = $db->query($sql)){
        die('There was an error running the query [' . $db->error . ']');
  }
  ...
  while($row = $result->fetch_assoc()){
    ...
    $stock[$row['id']] = array("retail" => $row['retail_price'], "cost" => $row['cost_price']); //store price in array indexed by id //NB retail_price or price ?
  ...
  */
  ...
  foreach ($quantities as $stock_id=>$num_sold) { //$key actuall == stock_id value)
    $price = $stock[$stock_id]['retail']; //$price is looked up against the stock info gathered earlier
    $cost = $stock[$stock_id]['cost'];
    $sql = "INSERT INTO `sales_record` VALUES ($event_record_id,$stock_id,$num_sold,$price,$cost)";
    if(!$result = $db->query($sql)){
        die('There was an error running the query [' . $db->error . ']');
    }
  }
